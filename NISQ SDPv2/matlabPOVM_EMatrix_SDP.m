function beta_result = matlabPOVM_EMatrix_SDP(E_matrix_in, R_matrix_in, ...
                                                slack_overlap)

    commonPath = "C:\Users\Ankith Mohan\Downloads\NISQ SDPv2\";
    for folder = ["cvx-w64", "QETLAB"]
    ...
    n_states = size(R_matrix); % number of states to be discriminated
    n_states = n_states(3);
    ...
    cvx_begin
        ...
        obj = 0;
        for i = 1:n_states
            obj = obj + real(trace(var_beta(:,:,i) * E_matrix ...
                                    * R_matrix(:,:,i) * E_matrix));
        end
        maximize obj
        subject to
            ...
            %     for j = 1:n_states
            %         if (i ~= j)
            %             real(trace(var_beta(:,:,i) * E_matrix ...
            %                           * R_matrix(:,:,j) ...
            %                           * E_matrix)) <= slack_overlap;
            %         end
            %     end
            % end
            ...
            for i = 1:n_states % sum over POVM
                ...
                for j = 1:n_states % sum over states
                    if (i ~= j)
                        unambig(i) = unambig(i) + real(trace(var_beta(:,:,i) ...
                                                            * E_matrix ...
                                                            * R_matrix(:,:,j) ...
                                                            * E_matrix));
                    end
            ...
            E_matrix - E_matrix * sum_beta ...
                * E_matrix == hermitian_semidefinite(n_expand_states)
    cvx_end
